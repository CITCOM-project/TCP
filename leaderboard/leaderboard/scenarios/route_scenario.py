...
SECONDS_GIVEN_PER_METERS = 3  # for timeout
...
class RouteScenario(BasicScenario):
    ...
    def __init__(
        ...
        ideal_number_of_drivers=None,
        ideal_number_of_walkers=None,
        ego_vehicle_model="vehicle.lincoln.mkz2017",
    ...
    def _update_ego_vehicle(self):
        ...
        ego_vehicle = CarlaDataProvider.request_new_actor(
            self.ego_vehicle_model, elevate_transform, rolename="hero", color=self.ego_vehicle_color
        )
    ...
    def _build_scenario_instances(
        ...
        for scenario_number, definition in enumerate(scenario_definitions):
            ...
            scenario_configuration.ego_vehicles = [
                ActorConfigurationData(self.ego_vehicle_model, ego_vehicle.get_transform(), "hero")
            ]
    ...
    def _initialize_actors(self, config):
        ...
        self.ideal_number_of_drivers = (
            self.ideal_number_of_drivers
            if self.ideal_number_of_drivers is not None
            else town_amount.get(config.town, 0)
        )
        self.ideal_number_of_walkers = (
            self.ideal_number_of_walkers
            if self.ideal_number_of_walkers is not None
            else town_amount.get(config.town, 0)
        )

        # assert self.ideal_number_of_drivers is not None, "None drivers"

        self.walker_thread = CarlaDataProvider.request_new_batch_walkers(
            "walker.*",
            self.ideal_number_of_walkers,
            carla.Transform(),
            autopilot=True,
            ...
            if scenario.scenario.behavior is not None:
                ...
                if route_var_name is not None:
                    ...
                    oneshot_idiom = oneshot_behavior(
                        name=name, variable_name=name, behaviour=scenario.scenario.behavior
                    )
